...
function cmanGetOk(argPos){
    ...
    // ---【デモ用】取得した位置の地図リンク編集 ---------------------------------------------------
    document.getElementById("tblMStaffGoogleMapLink").value = 'http://maps.google.com/maps?q=' + argPos.coords.latitude + ',' + argPos.coords.longitude;
    document.getElementById("tblMStaffLatitude").value = argPos.coords.latitude;
    document.getElementById("tblMStaffLongitude").value = argPos.coords.longitude;
...
function cmanGetErr(argErr){
    var wErrMsg = "";
    switch(argErr.code){
        case 1 : wErrMsg = "User denied the request for Geolocation.";break;
        case 2 : wErrMsg = "Location information is unavailable.";break;
        case 3 : wErrMsg = "The request to get user location timed out.";break;
    }
    if(wErrMsg == ""){wErrMsg = argErr.message;}

    // --- エラーメッセージ出力 --------------------------------------------------------------------
    document.getElementById("tblMStaffGoogleMapLink").value = wErrMsg;
...
function cmanPosGet(){
    ...
    if (typeof navigator.geolocation === 'undefined') {
        document.getElementById("tblMStaffGoogleMapLink").value = 'ブラウザが位置情報取得に対応していません';
        cmanGetDisplayCtrl('ERR');
        return false;
    }
